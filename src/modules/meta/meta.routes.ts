...
router
	...
	.post(authenticate, authorize(roles.systemAdmin), upload.single('image'), validateResource(metaSchema), upsertMeta)

router
	.route('/:resource/:id')
	.get(validateResource(getMetaSchema), getMetaById)
	.put(authenticate, authorize(roles.systemAdmin), upload.single('image'), validateResource(metaSchema), upsertMeta)
