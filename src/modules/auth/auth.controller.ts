...
export const sendOTP = asyncHandler(
  async (req: Request<object, object, SendOTPBody>, res: Response) => {
    const { countryCode, phone, resend = false } = req.body
    const phoneE164 = countryCode.startsWith('+')
      ? `${countryCode}${phone}`
      : `+${countryCode}${phone}`
    ...
    if (!phoneEnabled) {
      ...
      return res
        .status(200)
        .json(new ApiResponse(200, null, 'Phone verification disabled at the moment'))
    }
    ...
    try {
      ...
      logError(
        `Failed hashOTP: Hashing error`,
        err,
        { action: 'hashOTP', phoneE164: maskedPhone },
        req,
      )
    ...
    try {
      ...
      logInfo(
        'OTP sent',
        {
          action: 'waitForStatus',
          phoneE164: maskedPhone,
          sid: statusResult.sid,
...
export const verifyOTP = asyncHandler(
  async (req: Request<object, object, VerifyOTPBody>, res: Response) => {
    ...
    try {
      ...
      const err = error as Error
      logError(
        `Failed verifyOTP: OTP check error`,
        err,
        { action: 'verifyOTP', phoneE164: maskedPhone },
        req,
      )
  ...
  async (req: Request<object, object, RefreshTokenBody>, res: Response) => {
    ...
    try {
      ...
      logWarn(
        ...
        { action: 'verifyRefreshToken', error: err.message },
        req,
      )
    ...
    try {
      ...
      if (!providedToken || providedToken !== storedToken) {
        logWarn(
          'Refresh token mismatch or not found in storage',
          { action: 'getRefreshToken', userId },
          req,
        )
