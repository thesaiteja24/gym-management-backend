...
export const startConversation = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    logError(
      ...
      err,
      { action: 'startChat', error: err.message },
      req,
    )
  ...
  try {
    ...
    const err = error as Error
    logError(
      'Failed to set conversation in cache',
      err,
      { action: 'startChat', error: err.message },
      req,
    )
...
export const transcribeMessage = asyncHandler(async (req: Request, res: Response) => {
  ...
  logInfo(
    'Audio transcribed',
    { action: 'askCoach', transcriptLength: transcription.text.length },
    req,
  )
...
export const sendMessage = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!question || typeof question !== 'string' || !question.trim()) {
    logWarn(
      ...
      { action: 'answerQuestion', questionType: typeof question },
      req,
    )
  ...
  try {
    ...
    logError(
      'Failed to generate chat response',
      err,
      { action: 'answerQuestion', error: err.message },
      req,
    )
    throw new ApiError(500, 'Failed to generate chat response')
  }
  ...
  logInfo(
    ...
    { action: 'answerQuestion', transcriptLength: generatedText.text.length },
    req,
  )
  return res.json(
