...
// patch
export const updateMe = asyncHandler(async (req: Request<object, object, UpdateMeBody>, res: Response) => {
	const userId = req.user!.id
	...
	return res.status(200).json(new ApiResponse(200, formatUserResponse(updatedUser), 'Details updated successfully'))
})
...
export const updateMyProfilePic = asyncHandler(async (req: Request, res: Response) => {
	...
	try {
		...
		const err = error as Error
		logWarn('Failed to upload profile picture', { action: 'updateMyProfilePic', error: err.message }, req)
...
export const deleteMyProfilePic = asyncHandler(async (req: Request, res: Response) => {
	...
	try {
		...
		const err = error as Error
		logWarn('Failed to delete profile picture', { action: 'deleteMyProfilePic', error: err.message }, req)
