 ...
 */
async function instantiateUserWeek(
  tx: any,
...
function formatUserProgram(userProgram: any) {
  ...
  if (
    ...
    typeof formattedProgress.templateSnapshot.exercises === 'object'
  ) {
...
export const createProgram = asyncHandler(
  async (req: Request<object, object, CreateProgramBody>, res: Response) => {
    ...
    if (existing) {
      logWarn(
        'Program idempotency hit',
        { action: 'createProgram', clientId, programId: existing.id },
        req,
      )
    ...
    logInfo(
      ...
      { action: 'createProgram', id: program.id, title: program.title },
      req,
    )
...
export const getProgramById = asyncHandler(
  async (req: Request<{ programId: string }>, res: Response) => {
    ...
    logInfo(
      ...
      { action: 'getProgramById', programId, title: program.title },
      req,
    )
...
export const editProgram = asyncHandler(
  async (req: Request<{ programId: string }, object, CreateProgramBody>, res: Response) => {
    ...
    logInfo(
      ...
      { action: 'editProgram', programId, title: updatedProgram?.title },
      req,
    )

    return res.json(
      new ApiResponse(200, { program: updatedProgram }, 'Program updated successfully'),
    )
  },
)
...
export const startProgram = asyncHandler(
  async (
    req: Request<{ userId: string; programId: string }, object, StartProgramBody>,
    res: Response,
  ) => {
    ...
    if (userId !== req.user?.id) {
      logWarn(
        'You are not authorized to perform this action',
        { action: 'startProgram', programId, userId },
        req,
      )
      throw new ApiError(403, 'You are not authorized to perform this action')
    }
    ...
    if (duration > MAX_DURATION) {
      logWarn(
        'We allow only up to 16 weeks for now',
        { action: 'startProgram', programId, duration },
        req,
      )
    ...
    if (duration < MIN_DURATION) {
      logWarn(
        'Duration is less than minimum allowed',
        { action: 'startProgram', programId, duration },
        req,
      )
    ...
    return res.json(
      new ApiResponse(
        200,
        { userProgram: formatUserProgram(result) },
...
export const getUserProgramById = asyncHandler(
  async (req: Request<{ userId: string; userProgramId: string }>, res: Response) => {
    ...
    return res.json(
      new ApiResponse(
        200,
        { program: formatUserProgram(userProgram) },
        'User program fetched successfully',
      ),
    )
  },
)
  ...
  async (req: Request<{ userId: string }>, res: Response) => {
    ...
    return res.json(
      new ApiResponse(
        200,
        { program: formatUserProgram(userProgram) },
        'Active program fetched successfully',
      ),
    )
  },
)
...
export const listUserPrograms = asyncHandler(
  async (req: Request<{ userId: string }>, res: Response) => {
    ...
    return res.json(
      new ApiResponse(200, { programs: userPrograms }, 'User programs fetched successfully'),
    )
  },
)
